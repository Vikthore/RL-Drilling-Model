...
class DrillingEnv(gym.Env):
    ...
    def __init__(self, data_path=None):
        ...
        low=0.0, high=1.0, shape=(len(self.feature_columns),), dtype=np.float32  # <--- ADDED CLOSING PARENTHESIS AND COMMA HERE
        )

        # 3.2. Action Space
        self.action_space = spaces.Box(
            low=-0.1, high=0.1, shape=(3,), dtype=np.float32
        )  # Adjust WOB, RPM, MW within +/- 0.1 in scaled space
    ...
    def reset(self, seed=None, options=None):
        ...
        return initial_state, {}  # Return 1D observation and empty info dict
    ...
    def step(self, action):
        ...
        k1, k2, k3, k4, k5, k6 = 5, 0.5, 0.3, 0.1, 0.01, 0.01  # Constants
        ...
        next_observation = next_state_scaled  # For now, next_observation is just the scaled state.
